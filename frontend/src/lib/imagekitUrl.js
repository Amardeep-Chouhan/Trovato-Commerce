 ...
 */
function buildTrovatoTextLayer({ w, h }) {
  ...
  return `l-text,i-Trovato,fs-${fs},co-FFFFFF,bg-0F172A90,pa-8_12,lx-N14,ly-14,lap-top_right,l-end`;
}
...
function buildTrSegment({ w, h, q = 80, f = "auto", crop, watermark = false }) {
  ...
  return `${base}:${buildTrovatoTextLayer({ w, h })}`;
}
...
/**
 * Same optimizations as {@link imageKitOptimizedUrl} plus Trovatotext overlay (for share/download).
